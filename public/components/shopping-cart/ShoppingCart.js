...
import {
  ...
  clearCart,
  updateProduct,
} from "/public/components/util/CartActions.js";
...
function ShoppingCart() {
  ...
  const _checkoutButtonElement = _rootElement.querySelector(
    '[data-js="checkout-button"]'
  );
  const _cartTotalElement = _checkoutButtonElement.querySelector(
    '[data-js="cart-total"]'
  );
  const _cartQuantityElement = _rootElement.querySelector(
...
<div class="row g-4">
  ...
  function _createItemElement(item) {
    ...
    element.innerHTML = `<td class="d-flex">
        <img class="item-image me-3" src="/${
          item.image
        }" alt="Imagen que representa al producto en la fila del carrito de compras">
        <div class="">
    ...
    <td data-colname="Cantidad" class="text-end text-sm-center">
        <input type="number" data-js="item-quantity" class="text-center quantity-input d-inline-block form-control p-2" step="1" name="item-quantity-input" value="${
          item.quantity
        }" required min="1">
    </td>
    <td data-colname="Total" class="text-end text-sm-center item-total" data-js="item-total">${currencyFormat(
      item.total
    )}</td>
    ...
    itemQuantity.oninput = _onChangeItemQuantity.bind(
      itemQuantity,
      element,
      item
    );
  ...
  async function _processCartState() {
    ...
    if (!appState.getState().customer && cartItems.length > 0) {
  ...
  function _enableCheckoutButton() {
    ...
    _checkoutButtonElement.children[0].lastElementChild.children[0].classList.add(
      "d-none"
    );
    _checkoutButtonElement.children[0].lastElementChild.children[1].classList.remove(
      "d-none"
    );
  }
  ...
  function _showLoadingCheckoutButton() {
    ...
    _checkoutButtonElement.children[0].lastElementChild.children[0].classList.remove(
      "d-none"
    );
    _checkoutButtonElement.children[0].lastElementChild.children[1].classList.add(
      "d-none"
    );
  }
  ...
  async function _onDeleteItem(element, item) {
  ...
  async function _onChangeItemQuantity(element, item) {
    if (isSubmitting) return;
    ...
    element.querySelector('[data-js="item-total"]').textContent =
      currencyFormat(item.total);

    let cartTotal = 0;
    console.log(item);

    const addPersistentProduct = await updateProduct({
      id: item.id,
  ...
  function _onSubmitShoppingCart(event) {
    ...
    console.log(appState.getState().cart);
