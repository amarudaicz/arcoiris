import { getCart } from "/public/components/util/CartActions.js";
...
function AppState() {
  ...
  this.loadCart = async function () {
    const cart = await getCart();
    _self.state.cart = cart.data ? cart.data.items : [];
    this.saveState();
    this.notifyAll();
  };

  // Method to load the state of the app from the session storage
  this.loadState = async function () {
